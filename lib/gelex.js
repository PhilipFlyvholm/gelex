...
function LexerDefinition() {
    ...
    function createOrRule(data) {
        ...
        while (data.length)
            ...
            else if (data[0] === '{' && data.indexOf('}') > 0) {
                const p = data.indexOf('}');
                const name = data.substring(1, p);
                const rule = sets[name];
    ...
    function createRule(data, trfn) {
        ...
        while (data.length)
            if (data[0] === '[' && data.indexOf(']') > 0) {
                ...
                rules[rules.length - 1] = new AtLeastOneRule(rules[rules.length - 1]);
                data = data.substring(1);
            }
            else {
